...
| `productos/page.tsx` y `UbicacionDetalle.tsx` | Ponytail 2.3 evaluó consolidar loading/error/paginación en un hook `usePaginatedData` compartido — descartado | Similitud es de nombres, no de mecanismo: fuentes de datos distintas (fetch API route vs server action directo), loading compartido vs separado, arquitecturas de búsqueda distintas (AbortController vs debounceRef). Si se vuelve a evaluar, confirmar primero que ambos mecanismos converjan de verdad antes de abstraer |
| `lib/constants.ts` + `lib/sync/compras-anil.ts` | `BODEGA_ID` hardcodeado con IDs numéricos. Si las tablas `bodegas`/`modulos` se recrean y los serials quedan con IDs distintos, el sync inserta stock en la ubicación equivocada sin advertirlo | Defensa activa en `syncComprasAnil()`: valida al inicio que los IDs y nombres coincidan con las filas reales. Si falla, el sync aborta con error específico (qué bodega, qué ID esperado vs encontrado). Migrar `BODEGAS`/`MODULOS` a DB como solución definitiva cuando haya presupuesto para el slice completo |
